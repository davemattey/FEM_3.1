...
# choose run model from today TRUE or from past date FALSE
Model_run_today = False
Model_run_from = '25-11-2020'
# convert to datetime
Model_run_from_DT = datetime.datetime.strptime(Model_run_from,'%d-%m-%Y')

# set begin date for model run (today or a past date)
if Model_run_today == True: begin = date_today
else: begin = Model_run_from_DT

# Monthly lookup data for P-ET calcs
...
days_forward_risemodel = 14  # use to fix duration of forward model if RISE
days_forward_fallmodel = 28  # use to fix duration of forward model if LINEAR OR FALL

#  print initial conditions

if Model_run_today == True:
...
# Set windows for forward model
print('project model on rise days_forward_risemodel', days_forward_risemodel)
...
# select start date and create working dataframe 'df_search window'

# set date range
if Model_run_today == True:
    end_date = date_today
else:
    end_date = datetime.datetime.strptime(Model_run_from, '%d-%m-%Y')

start_date = end_date - datetime.timedelta(days=W_search)
print('Model run begins on ', start_date, ' and ends on ', end_date)

# slice date range and print
df_searchwindow = df4[df4["TIMESTAMP"].isin(pd.date_range(start_date, end_date))]
print('Slicing ', W_search, ' days from df4')
print(df_searchwindow)
df_searchwindow.to_csv('searchwindow.csv', index=True)

# development: print stuff
...
#  pull out datapoints at end of date range
...
# P_peaks = find_peaks(df_searchwindow['P_rolling_smooth'], height = ET_P_window, prominence = 10)
...
# make a new dataframe to store master list of recharge peaks in model window
global df_recharge_peaks
df_recharge_peaks = pd.DataFrame()  # make a dataframe

# fill new df with selected columns
for i in P_index:
    df_recharge_peaks['date_recharge_max'] = df_searchwindow.iloc[i, 0]
    df_recharge_peaks['P_recharge_max'] = df_searchwindow.iloc[i, 48]
    df_recharge_peaks['rate_recharge_max'] = df_searchwindow.iloc[i, 46]
...
if len(df_recharge_peaks.axes[0]) > 0:
    ...
    print("RECHARGE  P rolling max date, P, rate and depth ", date_recharge_max, f'{P_recharge_max:.3f}',
          f'{rate_recharge_max:.3f}', f'{depth_recharge_max:.3f}')
# else: print ("No peaks found")
else:
    No_peaks = True

if No_peaks == True: print("No peaks found")
if No_peaks == False:
...
if No_peaks == False:
    ...
    print("RECHARGE  P threshold  date, P, rate and depth ", date_recharge_threshold, f'{P_recharge_threshold:.3f}',
...
if No_peaks == False and bh_change_week > 0:
    ...
    print('######################################### LEVEL CHANGE RATE ############################################')
    # Find BH rate of change peaks if rate falling
    ...
    if no_rate_peaks == False:
        ...
        print("BH CHANGE  BH rate max date, P, rate and depth ", date_change_max, f'{P_change_max:.3f}',
              f'{rate_change_max:.3f}', f'{depth_change_max:.3f}')
...
print(
    '•••••••••••••••••••••••••••••••••••• FIND LATEST TREND AND SET FLAG TO SELECT MODEL ••••••••••••••••••••••••••••••')
...
if slope >= 2.5: EF_change = 'EF_change_rise'
if slope < 2.5 and slope > -2.5: EF_change = 'EF_change_flat'
if slope <= -2.5: EF_change = 'EF_change_fall'

print('EFFECTIVE RECHARGE  P_change  ACTUAL', EF_change)
...
print("CHANGE  BH rate min date, P, rate and depth ", date_change_min, f'{P_change_min:.3f}', f'{rate_change_min:.3f}',
...
print('Last rate peak date was ', date_change_max)
...
if No_peaks == True:
    ...
    if recharge == True and bh_change_week > 0: model = 'Rise'
...
# if recharge == False and P_total_today > ET_P_window: status_recharge = ' higher recharge may change the recent trend in groundwater level '
...
elif bh_change_today < .05 and bh_change_today > -.05:
    status_change = ' and is changing by '
elif bh_change_today < -.05 and bh_change_today > -.15:
    status_change = ' and is falling slowly at '
elif bh_change_today > .025 and bh_change_today < 0.15:
    status_change = ' and is rising slowly at '
elif bh_change_today > 0.15 and bh_change_today < 0.25:
...
elif bh_change_rate > -0.015 and bh_change_rate < 0.015:
...
elif bh_level_today > bh_low and bh_level_today < bh_mean:
    status_season = 'slightly lower than seasonal average'
elif bh_level_today < bh_mean + 1.5 and bh_level_today > bh_mean - 1.5:
    status_season = 'near seasonal average'
elif bh_level_today > bh_mean and bh_level_today < bh_high:
