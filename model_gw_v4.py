...
# set up validation loop using historic data
...
if not No_peaks:
    ...
    print('##################################### FIND RECHARGE THRESHOLD DATES ########################################')
    ...
    print("Conditions at P threshold  date, P-ET, change rate and depth ", date_recharge_threshold, f'{PminusET_recharge_threshold:.3f}',
...
if model == 'Fall':
    ...
    for x in range(1, math.trunc(days)):
        y = y_0 * math.exp(k * x)
        sum = sum + y
        new_depth = sum + depth_change_peak
...
new_row = [begin,model,proj_date_end,proj_depth_end]
