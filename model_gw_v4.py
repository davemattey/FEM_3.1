...
# set historical data limits
Model_run_from = '01-11-2021'
Model_run_interval = 3
Model_run_nsteps = 10
...
df_searchwindow.to_csv('searchwindow_1.csv', index=True)
...
if pd.isna(bh_change_today) == True: bh_change_today = 0

# bh_change_today = df_searchwindow.at[df_searchwindow.index[-1], 'BH_level'] - df_searchwindow.at[df_searchwindow.index[-2], 'BH_level']
bh_change_yesterday = df_searchwindow.at[df_searchwindow.index[-2], 'BH_level'] - df_searchwindow.at[
    df_searchwindow.index[-3], 'BH_level']
...
if not No_peaks:
    ...
    for i in BH_rate_index:
        ...
        df_BH_rate_peaks['P_rate_peak'] = df_searchwindow.iloc[i, 44]
        df_BH_rate_peaks['rate_rate_peak'] = df_searchwindow.iloc[i, 43]
        df_BH_rate_peaks['depth_rate_peak'] = df_searchwindow.iloc[i, 37]
    ...
    if No_rate_peaks == True: print("No rate peaks found")
    if No_rate_peaks == False:
        print('LEVEL RATE OF CHANGE peaks')
        print(df_recharge_peaks)
...
print('EFFECTIVE RECHARGE  P_change  ACTUAL', ER_change)
...
if slope >= 0.001: rate_change = 'r_change_rise'
if slope < 0.001 and slope > -0.001: rate_change = 'r_change_flat'
if slope <= -0.001: rate_change = 'r_change_fall'
...
if slope >= 0.05: depth_change = 'd_change_rise'
if slope < 0.05 and slope > -0.05: depth_change = 'd_change_flat'
if slope <= -0.05: depth_change = 'd_change_fall'
...
print('DATA TO BE USED IN MODEL CALCS')
print('FALL')
...
if recharge and ER_change == 'ER_change_rise':
    model = 'P_rising'
if recharge and P_change == 'P_change_flat':
    model = 'P_falling'
if not recharge and depth_change == 'd_change_flat':
    model = 'Linear'
if not recharge and depth_change == 'd_change_fall':
    model = 'Linear'
print('Model = ', model)
...
if model == 'P_falling':
    # override begin date as date_change_peak
...
if model == 'P_rising':
    ...
    # set begin date as date_change_peak
